...
async def analyze_dbt_file_for_iterative_query(
    ...
    """
    logger.info("🚀 Starting iterative dbt query analysis")
    logger.info(f"📊 Target database: {host}:{port}/{database}")
        ...
        while current_depth >= 0:
            ...
            try:
                ...
                if decision["decision"].lower().strip() == "yes":
                    ...
                    current_depth -= 1
                    logger.info(f"⬇️  Reducing depth to {current_depth} and retrying")
                    
            except Exception as depth_error:
                logger.error(f"❌ Error at depth {current_depth}: {depth_error}", exc_info=True)
                process_log.append({
                    "depth": current_depth,
                ...
                current_depth -= 1
                logger.warning(f"⬇️  Error occurred, reducing depth to {current_depth}")
        
        # Should never reach here, but just in case
        logger.error("🚫 Exhausted all depth levels without success")
        return {
            "status": "exhausted",
...
async def ask_ai_sufficiency_decision(
    ...
    """
    logger.debug(f"🤖 AI Sufficiency Check - Depth {current_depth}")
    logger.debug(f"📊 Evaluating {len(tables)} tables with {len(column_metadata)} columns")
    ...
    logger.debug(f"📋 Table summary prepared with {sum(len(t['columns']) for t in table_summary)} total columns")
