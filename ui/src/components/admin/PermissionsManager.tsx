...
export function PermissionsManager() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between bg-[#FFB24C]/10 border border-[#FFB24C]/20 p-4 rounded-xl">
          <div>
            <h3 className="font-semibold flex items-center gap-2 text-lg">
              <ShieldCheck className="w-5 h-5 text-[#FFB24C]" />
              Admin Role Management
            </h3>
            <p className="text-sm text-muted-foreground mt-1">
        ...
        <div className="mt-4 max-w-sm">
          <Card className="border-[#FFB24C]/20 bg-[#FFB24C]/5 hover:border-[#FFB24C]/40 transition-all duration-300">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg flex items-center gap-2">
                <Crown className="w-5 h-5 text-[#FFB24C]" />
                Administrators
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex gap-3 mb-6">
                <Badge variant="secondary" className="flex items-center gap-1.5 py-1.5 px-3 bg-[#FFB24C]/10 text-[#935900] dark:text-[#FFB24C] hover:bg-[#FFB24C]/20">
                  <Users className="w-3.5 h-3.5" />
              ...
              <div className="flex gap-2 w-full">
                <Button
                  variant="outline"
                  className="flex-1 border-[#FFB24C]/30 hover:bg-[#FFB24C]/10 hover:border-[#FFB24C]/50"
                ...
                <Button
                  variant="outline"
                  className="flex-1 border-[#FFB24C]/30 hover:bg-[#FFB24C]/10 hover:text-[#935900] dark:text-[#FFB24C] hover:border-[#FFB24C]/50"
                    ...
                    filteredAuthUsers.length > 0 ? (
                      filteredAuthUsers.map(u => (
                        <div key={u.id} className="flex items-center justify-between p-3 rounded-lg border border-border/50 bg-surface/30 hover:bg-surface/50 transition-colors">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => handleRemoveUser(u.id)} className="h-8 px-2 text-[#F16C6C] hover:text-[#c03232] hover:bg-[#F16C6C]/10">
                    ...
                    filteredAuthGroups.length > 0 ? (
                      filteredAuthGroups.map(g => (
                        <div key={g.id} className="flex items-center justify-between p-3 rounded-lg border border-border/50 bg-surface/30 hover:bg-surface/50 transition-colors">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => handleRemoveGroup(g.id)} className="h-8 px-2 text-[#F16C6C] hover:text-[#c03232] hover:bg-[#F16C6C]/10">
                            <Trash2 className="w-4 h-4" />
                          </Button>
                        </div>
                      ))
      ...
      <Dialog open={adminDialogOpen} onOpenChange={(open) => !open && setAdminDialogOpen(false)}>
        <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col p-0 overflow-hidden bg-background">
          <div className="p-6 border-b border-[#FFB24C]/20 bg-[#FFB24C]/5">
            <DialogTitle className="text-2xl flex items-center gap-2 mb-2">
              <ShieldCheck className="w-6 h-6 text-[#FFB24C]" />
              Admin Role Management
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="flex border-b border-border/50">
            <button
              className={`flex-1 py-3 text-sm font-medium border-b-2 transition-colors ${adminDialogMode === 'view' ? 'border-[#FFB24C] text-[#FFB24C] bg-[#FFB24C]/5' : 'border-transparent text-muted-foreground hover:bg-surface/50'}`}
              onClick={() => { setAdminDialogMode('view'); setAdminSearchQuery(''); }}
            >
              Current Admins
            </button>
            <button
              className={`flex-1 py-3 text-sm font-medium border-b-2 transition-colors ${adminDialogMode === 'add' ? 'border-[#FFB24C] text-[#FFB24C] bg-[#FFB24C]/5' : 'border-transparent text-muted-foreground hover:bg-surface/50'}`}
          ...
          <div className="p-6 flex-1 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                <h4 className="flex items-center gap-2 font-semibold mb-4 text-foreground/80 pb-2 border-b">
                  <Users className="w-4 h-4 text-[#FFB24C]" />
                  Users
                </h4>
                <div className="space-y-2">
                  {adminDialogMode === 'view' ? (
                    filteredAdminUsers.length > 0 ? (
                      filteredAdminUsers.map(u => (
                        <div key={u.id} className="flex items-center justify-between p-3 rounded-lg border border-[#FFB24C]/20 bg-[#FFB24C]/5 hover:bg-[#FFB24C]/10 transition-colors">
                          <div className="flex items-center gap-2">
                            <ShieldCheck className="w-4 h-4 text-[#FFB24C]" />
                            <span className="font-medium">{u.username}</span>
                          </div>
                          <Button variant="ghost" size="sm" onClick={() => handleRevokeAdmin(u.id)} disabled={adminSaving}
                            className="h-8 px-2 text-[#F16C6C] hover:text-[#c03232] hover:bg-[#F16C6C]/10">
                            <Trash2 className="w-4 h-4" />
                          </Button>
                        </div>
                      ))
                      ...
                      filteredNonAdminUsers.map(u => (
                        <div key={u.id} className="flex items-center justify-between p-3 rounded-lg border border-border/50 bg-surface/30 hover:bg-surface/50 transition-colors">
                          ...
                          <Button variant="outline" size="sm" onClick={() => handleGrantAdmin(u.id)} disabled={adminSaving}
                            className="h-8 px-3 text-[#935900] dark:text-[#FFB24C] border-[#FFB24C]/30 hover:bg-[#FFB24C]/10 hover:border-[#FFB24C]/50">
                            <Plus className="w-4 h-4 mr-1" /> Add
                          </Button>
                        </div>
                      ))
              ...
              <div>
                <h4 className="flex items-center gap-2 font-semibold mb-4 text-foreground/80 pb-2 border-b">
                  <UsersRound className="w-4 h-4 text-[#FFB24C]" />
                  Groups
                </h4>
                <div className="space-y-2">
                  {groups.length === 0 ? (
                    ...
                    filteredAdminGroups.length > 0 ? (
                      filteredAdminGroups.map(g => (
                        <div key={g.id} className="flex items-center justify-between p-3 rounded-lg border border-[#FFB24C]/20 bg-[#FFB24C]/5 hover:bg-[#FFB24C]/10 transition-colors">
                          <div className="flex items-center gap-2">
                            <ShieldCheck className="w-4 h-4 text-[#FFB24C]" />
                            <span className="font-medium">{g.name}</span>
                          </div>
