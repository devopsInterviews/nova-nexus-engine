              ...
              <div className="text-sm bg-gray-50 p-3 rounded">
                ...
                <div className="flex items-center space-x-4 mb-2">
                  <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${
                    ...
                    endpoints.find(e => `${e.method} ${e.path}` === selectedEndpoint)?.method === 'POST' ? 'bg-success/15 text-success' :
                    endpoints.find(e => `${e.method} ${e.path}` === selectedEndpoint)?.method === 'PUT' ? 'bg-yellow-100 text-yellow-800' :
                    endpoints.find(e => `${e.method} ${e.path}` === selectedEndpoint)?.method === 'DELETE' ? 'bg-destructive/15 text-destructive' :
            ...
            <div className="text-sm">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`px-2 py-1 rounded text-xs ${
                  response.status === 200 ? 'bg-success/15 text-success' : 'bg-destructive/15 text-destructive'
                }`}>
                  {response.status}
                </span>
              </div>
              ...
              <div key={test.id} className="flex items-center justify-between p-3 border rounded">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button 
                    ...
                    className="inline-flex items-center space-x-2 px-4 py-2 bg-destructive text-destructive-foreground text-sm font-medium rounded-lg shadow-md hover:bg-destructive/90 hover:shadow-lg transform hover:scale-105 transition-all duration-200"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
